...
document.addEventListener("alpine:init", () => {
	...
	Alpine.data("tools", () => ({
		search() {
			// TODO: Rebuild this to reconstruct data.
		...
		bubbleCopy(event: MouseEvent) {
			const t = event.target;
			if (t && t instanceof Element) {
				const div = t.querySelector("[data-selector='arrows']");
				if (div) {
					div.classList.add("hideArrows", "countdown");
					div.querySelectorAll("span").forEach((element) => {
						for (let i = 1; i <= 8; i++) {
							element.classList.remove("fadeinArrow" + i);
						}
					});
				}
				t.addEventListener(
					...
					() => {
						if (div) {
							div.classList.remove("hideArrows", "countdown");
							div.querySelectorAll("span").forEach(
								(element, index) => {
